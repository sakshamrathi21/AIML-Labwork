...
class SKLearnDecisionTree:
    ...
    def train_model(self, criterion="entropy"):
        ...
        tree = DecisionTreeClassifier(random_state=42, criterion=criterion)
        
        tree.fit(self.X_train, self.Y_train)
        self.tree = tree

        yhat_tree = tree.predict(self.X_test)
        yhat_tree_train = tree.predict(self.X_train)
        
        results = {"train_accuracy": metrics.accuracy_score(self.Y_train, yhat_tree_train), "test_accuracy": metrics.accuracy_score(self.Y_test, yhat_tree)}
        ...
        for max_depth in range(1, 11):
            for min_samples_split in range(3, 31, 3):
                for min_samples_leaf in range(1, 11):
                    ...
                    yhat_tree = tree.predict(self.X_test)
                    yhat_tree_train = tree.predict(self.X_train)
                    train_accuracies.append(metrics.accuracy_score(self.Y_train, yhat_tree_train))
                    test_accuracies.append(metrics.accuracy_score(self.Y_test, yhat_tree))
