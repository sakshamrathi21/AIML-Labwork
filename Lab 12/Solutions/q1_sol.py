...
def psne_gen(num_players, strategy, util_matrix):
    ...
    for i in range(len(allpermut)):#take each state 
        flag=0
        for j in range(len(allpermut[i])): #which is same as num_players
            ...
            for l in range(len(allpermut)):
                ...
                for k in range(len(allpermut[i])):
                    if(allpermut[l][k]==allpermut[i][k]): #for all others match them
                        matchflag+=1
                    elif j==k and l!=i:#hard update current one
                        matchflag+=1
        ...
        if(flag==num_players):#max for all players
...
def msne_gen(num_players, strategy, util_matrix):
    ...
    num = util_matrix[1][1][1] - util_matrix[0][1][1]
    den = util_matrix[0][0][1] - util_matrix[0][1][1] - util_matrix[1][0][1] + util_matrix[1][1][1]
    p = num / den

    num = util_matrix[1][1][0] - util_matrix[0][1][0]
    den = util_matrix[0][0][0] - util_matrix[0][1][0] - util_matrix[1][0][0] + util_matrix[1][1][0]
    q = num / den
