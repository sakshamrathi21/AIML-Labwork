...
def QP_solver(X, Y):
    ...
    """
    ###############################################################
    # Implement QP solver here
    ###############################################################
    # return the vector w which is the solution (as a numpy array)
    X_t = X.transpose()
    P = X_t @ X
    q = -X_t @ Y
    A = solvers.qp(matrix(P), matrix(q))
    # return the weights w which is the solution of the QP (as a numpy array (2, 1) dimensions)
    return np.array(A['x'])
