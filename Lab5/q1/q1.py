...
class Perceptron():
    ...
    def fit(self, x, y, plot_flag, lr = 0.001, epochs = 100):
        ...
        for epoch in range(epochs):
            ...
            wrong_weights = np.where(y_pred!= y)[0]
            # print(np.where(y_pred!=y))
            delta = lr*np.dot(y[wrong_weights], x[wrong_weights])
            self.weights += delta
            # self.weights = self.weights + lr*np.sum(np.dot(np.transpose(X), y_pred)[])
...
if __name__ == "__main__":
    ...
    weights_history, average_weights_history = p.fit(x_train, y_train, True)
