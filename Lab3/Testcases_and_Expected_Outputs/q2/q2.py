...
def lasso_loss(X, Y, w, lambd):
    ...
    loss = (1/difference.size()[0])*((torch.transpose(difference, 0, 1)@difference)[0][0])
    n = (w.size()[0])
    # print(loss)
    for i in range(n):
        loss += lambd*abs(w[i][0])
...
def lasso_loss_derivative(X, Y, w, lambd):
    ...
    # print(Y.size())
    # print(w.size())
    derivative = ((2/Y.size()[0])*(((torch.transpose(X, 0, 1)@X)@w) - torch.transpose(X, 0, 1)@Y) + torch.mul(torch.sign(w), lambd))
    ### YOUR CODE ENDS HERE ###
