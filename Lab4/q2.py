...
class MultiClassLogisticRegression:
    ...
    def predict(self, X):
        ...
        # print(X)
        pre_vals = np.dot(X, np.transpose(self.weights))
    #   ## YOUR CODE ENDS HERE ###
    ...
    def softmax(self, z):
        ...
        sum_exp_scores = np.sum(post_softmax, axis=1, keepdims=True)
        post_softmax /= sum_exp_scores
        # print("hello")
        # print(post_softmax)
        ### YOUR CODE ENDS HERE ###
        return post_softmax
    ...
    def cross_entropy(self, y, probs):
        ...
        # print(np.shape(y))
        m, K = np.shape(probs)
        post_cross_entropy = np.sum(y * np.log(probs))
        # for i in range(m):
        #     for k in range(K):
        #         post_cross_entropy += y[i][k] * np.log(probs[i][k])

        post_cross_entropy *= (-1/m)
        ### YOUR CODE ENDS HERE ###
