		...
		for outcome in np.unique(self.y_train):
			...
			for i in self.y_train:
				...
				if (i == outcome):
					count+=1
			# print(count, total)
			self.class_priors[outcome] = count/total #TODO_2    Remove 0 and write your code 
    		### YOUR CODE ENDS HERE ###
	...
	def predict(self, X):
		...
		for query in X:
			...
			for outcome in np.unique(self.y_train):
				...
				# print(evidence)
				# print(prior, likelihood, evidence)
				posterior = prior*likelihood/evidence #TODO_2         Remove 0 and get posterior

				probs_outcome[outcome] = posterior
				# print(posterior)
			# print(type(probs_outcome))
			result = max(probs_outcome, key=lambda k: probs_outcome[k]) #TODO_3          Remove None and get the Max
