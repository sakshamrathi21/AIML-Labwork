...
def cost_logreg(X, y, w, w_0):
    ...
    ### YOUR CODE BEGINS HERE ###
    # print("hello")
    # print(logistic(np.dot(X, w)+w_0))
    y_dash = logistic(np.dot(X, w)+w_0)
    for i in range(m):
...
def grad_logreg(X, y, w, w_0):
    ...
    ### YOUR CODE BEGINS HERE ###
    sigmoid_values = 1 / (1 + np.exp(-(np.dot(X, w) + w_0)))
    grad_w = np.dot((sigmoid_values - y), X)
    grad_w_0 = np.sum(sigmoid_values - y)
    grad_w_0 /= m
    grad_w /= m
    # for i in range(m):
    #     grad_w += (1/(1 + np.exp(-np.dot(X[i], w) - w_0)) - y[i])*X[i]
    #     grad_w_0 += (1/(1 + np.exp(-np.dot(X[i], w) - w_0)) - y[i])
    # grad_w_0*= (1/m)
    # grad_w *= (1/m)
    ### YOUR CODE ENDS HERE ###
