...
def lasso_loss(X, Y, w, lambd):
    ...
    n,d = X.shape
    loss = 1/n*torch.sum(torch.square(X@w - Y)) + lambd*torch.sum(torch.abs(w))
...
def lasso_loss_derivative(X, Y, w, lambd):
    ...
    n,d = X.shape
    derivative = 2/n*torch.t(X)@(X@w - Y) + lambd*torch.sign(w)
...
if __name__ == '__main__':
    ...
    ### UNCOMMENT & RUN THE CODE BELOW AFTER COMPLETING  functions lasso_loss_derivative(X, Y, w) and train_model(X_train, Y_train, w, eta)
