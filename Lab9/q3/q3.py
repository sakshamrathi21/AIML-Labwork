...
def kernel_2(a,b, degree=2):
    '''
    This kernel has an important hyperparameter which controls upto what dimension non linearity should be mapped
    '''
    out = None
    # TODO
    out = (1+np.dot(a, b.T))**degree
    # out = np.exp(-np.linalg.norm(a - b) / sigma)
    # END TODO
    return out


def kernel_3(a, b, gamma=1):
    '''
    RBF Kernel (Gaussian Kernel)
    '''
    out = None
    # TODO
    dist = np.linalg.norm(a[:, np.newaxis] - b, axis=2) ** 2
    out = np.exp(-gamma * dist)
    # END TODO
    return out
