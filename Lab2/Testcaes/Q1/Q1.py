...
def w_closed_form(X, Y):
    ...
    # print(torch.transpose(X, 0, 1)@X)
    w_closed = (torch.linalg.inv(torch.transpose(X, 0, 1)@X)@torch.transpose(X, 0, 1))@Y
...
def l2_loss(X, Y, w):
    ...
    # print(n)
    for i in range(n):
        sum += pow(Y[i]-torch.transpose(w, 0, 1)@X[i], 2)
        # print(sum)
    loss = (1/n)*sum
...
def l2_loss_derivative(X, Y, w):
    ...
    d = X.size()[1]
    for i in range(n):
        sum += (Y[i]-torch.transpose(w, 0, 1)@X[i])*X[i]
        # print("Hello")
        # print(sum)
    derivative = sum*(-2/n)
    derivative = derivative.view(d, 1)
    # print(derivative.view)
...
def train_model(X_train, Y_train, X_test, Y_test, w, eta):
    ...
    # print(np.array([l2_loss(X_train, Y_train, w)])[0][0])
    # print(old_loss)
    
    while(abs(old_loss-np.array([l2_loss(X_train, Y_train, w)])[0][0]) >= epsilon):
        print(abs(old_loss-np.array([l2_loss(X_train, Y_train, w)])[0][0]))
       
        old_loss = np.array([l2_loss(X_train, Y_train, w)])[0][0]
        dw = l2_loss_derivative(X_train, Y_train, w)
        # print(w.shape)
        w = torch.sub(w, eta*dw)
        test_err.append(l2_loss(X_test, Y_test, w))
        # print(l2_loss(X_test, Y_test, w))
    # print(w)
