...
class History:
            ...
            for i in range(3):
                if self.board[i] == self.board[i + 3] == self.board[i + 6] == player:
                    if player == "x":
                        ...
                        return -1
                if self.board[3 * i] == self.board[3 * i + 1] == self.board[3 * i + 2] == player:
                    if player == "x":
                        return 1
                    else:
                        return -1
    ...
    def is_draw(self):
        ...
        if self.is_win() == 0 and "0" not in self.board:
            return 1
        else:
            return 0
...
def backward_induction(history_obj):
    ...
    action_distribution = {str(action): 0 for action in range(9)}
    if best_action is not None:
        action_distribution[str(best_action)] = 1
    if history_obj.player == 'x':
        strategy_dict_x[history_key] = action_distribution
    else:
        strategy_dict_o[history_key] = action_distribution
    return best_utility
