...
def w_closed_form(X, Y):
    ...
    w_closed = torch.round(torch.linalg.pinv(X)@Y,decimals = 4)
...
def transform_features(X, degree=1):
    ...
    phi_X = np.concatenate([X**i for i in range(0, degree+1)], axis=1)
...
def l2_loss(X, Y, w):
    ...
    n,d = X.shape
    loss = 1/n*torch.sum(torch.square(X@w - Y))
...
def l2_loss_derivative(X, Y, w):
    ...
    n,d = X.shape
    derivative = 2/n*torch.t(X)@(X@w - Y)
